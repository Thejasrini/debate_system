...
def is_field_aware_consumer_record(row, dataset_name):
    """
    Field-aware filtering rule:
    Ensures Consumer Protection Law is the PRIMARY domain, avoiding false matches in boilerplate templates.
    """
    ...
    if "adaption-indian-legal-triage-samples-v4" in dataset_name:
        ...
        # Must explicitly state Legal area: Consumer law / Consumer protection as primary focus
...
def normalize_record(row, dataset_name, idx):
    ...
    # Classify source type: case_law vs secondary_legal_qa
...
def run_pipeline():
    print("==================================================")
    print("LEXAGENT HUGGINGFACE CLEAN CONSUMER PIPELINE")
    print("==================================================\n")
    
    raw_dataset_stats = {}
    normalized_all_records = []
    
    # Step 1 & 2 & 3: Load, Filter Field-Aware, Normalize
    for item in DATASETS_TO_PROCESS:
        ds_name = item["name"]
        config = item["config"]
        ds_label = f"{ds_name} ({config})" if config else ds_name
        print(f"📥 Loading & filtering dataset: {ds_label}...")
        ...
        try:
            ...
            print(f"   -> Raw: {total_rows} | Consumer Filtered: {len(consumer_records)}\n")
    ...
    # Step 4 & 5 & 6: Robust Deduplication BEFORE Train/RAG/Eval Split
    seen_hashes = set()
    unique_records = []
    ...
    for record in normalized_all_records:
        ...
        # Hash facts + title/question snippet for robust deduplication
    ...
    # Step 7: Deterministic 80/20 Split AFTER Deduplication
    random.seed(42)
    ...
    # Step 8: Prevent Evaluation Leakage Check
