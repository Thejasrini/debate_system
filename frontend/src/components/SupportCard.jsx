    ...
    <div className="docket-card petitioner-panel">
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "16px", paddingBottom: "10px", borderBottom: "1px solid var(--border-hairline)" }}>
        <h3 className="font-serif" style={{ color: "#52B788", fontSize: "1.15rem", margin: 0, display: "flex", alignItems: "center", gap: "8px" }}>
          <span>🟢</span> PETITIONER COUNSEL (Consumer Advocates)
        </h3>
        <span className="font-mono" style={{ backgroundColor: "rgba(46, 92, 78, 0.3)", color: "#52B788", padding: "3px 10px", borderRadius: "12px", fontSize: "0.8rem", border: "1px solid rgba(82, 183, 136, 0.4)" }}>
          DEBATE POINTS ({debatePoints.length || argsList.length})
        </span>
      </div>
      ...
      {/* Numbered Debate Points Summary Banner */}
      {debatePoints.length > 0 && (
        <div style={{ marginBottom: "16px", padding: "12px", backgroundColor: "rgba(46, 92, 78, 0.18)", borderRadius: "6px", border: "1px solid rgba(82, 183, 136, 0.35)" }}>
          <div className="font-mono" style={{ color: "#52B788", fontSize: "0.8rem", textTransform: "uppercase", letterSpacing: "0.5px", marginBottom: "8px", fontWeight: "bold" }}>
            🔥 NUMBERED LEGAL DEBATE POINTS
          </div>
          <ul style={{ margin: 0, paddingLeft: "18px", color: "var(--text-parchment)", fontSize: "0.9rem", lineHeight: "1.6" }}>
            {debatePoints.map((pt, i) => (
              <li key={i} style={{ marginBottom: "4px" }}>
                <strong>{typeof pt === "string" ? pt : `Point ${i + 1}`}</strong>
              </li>
            ))}
          </ul>
        </div>
      ...
      {argsList.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: "14px", marginBottom: "16px" }}>
          {argsList.map((arg, idx) => (
            <div key={idx} style={{ padding: "14px", backgroundColor: "rgba(46, 92, 78, 0.12)", borderRadius: "6px", border: "1px solid rgba(82, 183, 136, 0.25)" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "6px" }}>
                <span className="font-mono" style={{ color: "#52B788", fontSize: "0.85rem", fontWeight: "bold", textTransform: "uppercase" }}>
                  {arg.point_number || `Point ${idx + 1}`}: {arg.issue || "Consumer Claim"}
                </span>
              </div>
              ...
              {Array.isArray(arg.legal_basis) && arg.legal_basis.length > 0 && (
                <div style={{ display: "flex", flexWrap: "wrap", gap: "6px", marginBottom: "8px" }}>
                  {arg.legal_basis.map((lb, i) => (
                    <span key={i} className="font-mono" style={{ backgroundColor: "rgba(46, 92, 78, 0.3)", color: "#52B788", padding: "2px 8px", borderRadius: "4px", fontSize: "0.78rem" }}>
              ...
              {arg.strength && (
                <div style={{ fontSize: "0.82rem", color: "#6ee7b7", marginTop: "4px" }}>
                  💪 <strong>Strength:</strong> {arg.strength}
                </div>
              )}
              {arg.weakness && (
                <div style={{ fontSize: "0.82rem", color: "#fcd34d", marginTop: "2px" }}>
                  ⚠️ <strong>Weakness:</strong> {arg.weakness}
                </div>
              )}
            </div>
          ))}
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "12px", marginBottom: "14px", fontSize: "0.85rem" }}>
        {strengths.length > 0 && (
          <div style={{ padding: "10px", backgroundColor: "rgba(46, 92, 78, 0.15)", borderRadius: "4px", border: "1px solid rgba(82, 183, 136, 0.3)" }}>
            <strong style={{ color: "#52B788" }}>💪 Overall Strengths:</strong>
      ...
      {/* Proof Documents to Submit in Follow-Up Chat */}
      {missingEv.length > 0 && (
        <div style={{ padding: "10px 14px", backgroundColor: "rgba(201, 169, 97, 0.12)", borderRadius: "4px", border: "1px solid rgba(201, 169, 97, 0.3)" }}>
          <div className="font-mono text-brass" style={{ fontSize: "0.75rem", textTransform: "uppercase", marginBottom: "4px" }}>
            📄 Evidence to Provide in Continuous Chat
          </div>
          <ul style={{ margin: 0, paddingLeft: "16px", fontSize: "0.85rem", color: "var(--text-parchment)" }}>
